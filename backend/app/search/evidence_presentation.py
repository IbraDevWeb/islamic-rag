...
def _structure_provenance(result: EvidenceSearchResult) -> tuple[str, str]:
...
def _bibliographic_provenance(result: EvidenceSearchResult) -> dict[str, Any]:
...
def build_evidence_payload(
    ...
    for rank, result in enumerate(items, start=1):
        ...
        work_display = bibliography.title_ar if bibliography else result.work_title

        payload["results"].append(
            {
                "rank": rank,
                "score": round(result.score, 8),
                "source_score": round(result.source_score, 8),
                "variant_ranks": list(result.variant_ranks),
                "citation": {
                ...
                "passage_original": result.text_original,
